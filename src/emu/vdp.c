...
void render_line_noscale(int line);

extern uint8 tms_palette[16][3];
...
void vdp_reg_w(uint8 r, uint8 d)
{
    ...
    switch(r)
    {
        ...
        case 0x02: /* Name Table Base Address */
            vdp.ntab = (d << 10) & 0x3800;
            vdp.pn = (d << 10) & 0x3C00;
            viewport_check();
            break;

        case 0x03: /* Color Table Base Address */
            vdp.ct = (d << 6) & 0x3FC0;
            break;

        case 0x04: /* Pattern Generator Table Base Address */
            vdp.pg = (d << 11) & 0x3800;
            break;
        ...
        case 0x06: /* Sprite Pattern Generator Table Base Address */
            vdp.sg = (d << 11) & 0x3800;
            break;

        case 0x07: /* Overscan/Backdrop Color */
            vdp.bd = (d & 0x0F);
            break;
    }
}
...
//    {
        ...
        //line = (vdp.line + 1) % vdp.lpf;
//        render_line_noscale((vdp.line + 1) % vdp.lpf);
  //  }
...
uint8 vdp_read(uint8 offset)
{
    ...
    switch(offset & 1)
    {
        ...
        case 1: /* Status flags */
            ...
            if(temp & 0x20)
            {
                hc = hc_256[(cyc + 1) % CYCLES_PER_LINE];
        ...
        case 0: /* Data port */
            ...
            switch(vdp.code)
            {
                ...
                case 3: /* CRAM write */
                    if(vdp.addr & 1)
                    {
                        vdp.cram_latch = (vdp.cram_latch & 0xFF) | (data << 8);
                        vdp.cram[(vdp.addr & 0x3E)] = vdp.cram_latch & 0xFF;
                        vdp.cram[(vdp.addr & 0x3E) | 1] = (vdp.cram_latch >> 8) & 0xFF;
                        palette_sync((vdp.addr >> 1) & 0x1F);
                    }
                    else
                    {
                        vdp.cram_latch = (vdp.cram_latch & 0xFF00) | data;
                    }
                    vdp.buffer = data;
                    break;
            }
