...
extern void psg_stereo_w(int data);

io_state io_lut[2][256] ALIGN;
io_state *io_current ALIGN;
...
static uint8 device_r(uint8 port)
{
    ...
    switch(sms.device[port])
    {
        ...
        case DEVICE_PAD2B:
            ...
            {
                // Bit 0
                if(input[port].pad & INPUT_LEFT) temp &= ~0x1;
                // Bit 1
                if(input[port].pad & INPUT_RIGHT) temp &= ~0x2;
                // Bit 2
                if(input[port].pad & INPUT_BUTTON2)
                {
                    temp &= ~0x4;  /* TL */
                }
                // Bit 3
                if(input[port].pad & INPUT_BUTTON1)
                {
                    temp &= ~0x8;  /* TR */
                }
                /* RESET */
                if(input[port].system & INPUT_RESET)
                {
                    temp &= ~0x10;
                }
            }
            ...
            {
                // Bit 0
                if(input[port].pad & INPUT_UP) temp &= ~0x1;
                // Bit 1
                if(input[port].pad & INPUT_DOWN) temp &= ~0x2;
                // Bit 2
                if(input[port].pad & INPUT_LEFT) temp &= ~0x4;
                // Bit 3
                if(input[port].pad & INPUT_RIGHT) temp &= ~0x8;
                // Bit 4
                if(input[port].pad & INPUT_BUTTON2)
                {
                    temp &= ~0x10;  /* TL */
...
uint8 pio_port_r(uint8 offset)
{
    ...
    /* 
      If I/O chip is disabled, reads return last byte of instruction that read the I/O port.
    */
    if(sms.memctrl & 0x04)
    {
    ...
    switch(offset & 1)
    {
        ...
        */
        case 0:
            ...
            temp = device_r(0);
            /* Read Up/Down from port B */
            // Bit 6
            if(input[1].pad & INPUT_UP) temp &= ~0x40;
            // Bit 7
            if(input[1].pad & INPUT_DOWN) temp &= ~0x80;

            // /* Read I/O port B low pins (Game Gear is special case with some extras) */
            // if(IS_GG)
            // {
                // temp |= sio_r(0) & 0x80;
// //            if(IS_GG)
...
// //                temp |= ((sio_r(1) & 0x03) << 6);
        ...
        /*
          ...
          Bit  Function:
          --------------
          7  Port B TH pin input (0x80)
          6  Port A TH pin input (0x40)
        ...
        */
        ...
        case 1:
            ...
            /* Read I/O port A pins */
            temp = device_r(1) & 0x1f;
//            temp |= (device_r(0) & 0xc0);
...
uint8 sio_r(uint8 offset)
{
    ...
    switch(offset)
    {
        case 0: /* Input port #2 */
            temp = 0xE0;
            if(input[0].system & INPUT_START)       temp &= ~0x80;
...
uint8 coleco_pio_r(uint8 port)
{
    ...
    {
        /* Joystick */
        if(input[port].pad & INPUT_UP) temp &= ~1;
        else if(input[port].pad & INPUT_DOWN) temp &= ~4;
        if(input[port].pad & INPUT_LEFT) temp &= ~8;
        else if(input[port].pad & INPUT_RIGHT) temp &= ~2;
        /* Left Button */
        if(input[port].pad & INPUT_BUTTON1) temp &= ~0x40;
    }
    else
    {
        /* KeyPad (0-9,*,#) */
        ...
        /* Right Button */
        if(input[port].pad & INPUT_BUTTON2) temp &= ~0x40;
    }
    return temp;
}
